      ...
      <div class="col-md-12 col-xs-12 col-sm-12">
        <div class="people-img-text">
          <div class="signin_header">
            <h1>{{ empty($category) ? 'Add' : 'Update'}} category</h1>
          </div>
          <div class="signinfrom-area">
            <form method="POST" action="{{ route('supplier.category.submit') }}" enctype="multipart/form-data">
              @csrf
              {{-- <input type="hidden" name="categoryId" value="{{ !empty($category) ? $category->id : '' }}"> --}}
                <div class="form-group">
                <label>Category Type {{Helper::errorField('category_type',$errors)}}</label>
                <select name="category_type" class="select2_parent" id="category_type">
                  @if(count($categoryType) > 0)
					<option value="">Select Shop</option>
                    @foreach($categoryType as $catType)
                      @if(!empty($product) && $catType->id == $product->category_type)
                        <option value="{{ $catType->id }}" selected="">{{ $catType->flag_value }}</option>
                      @else
                        <option value="{{ $catType->id }}">{{ $catType->flag_value }}</option>
                      @endif
                    @endforeach
                  @endif
                </select>
              ...
              <div class="form-group">
                <label>Category Name {{Helper::errorField('category_name',$errors)}}</label>
                <input class="form-control" name="category_name" placeholder="Enter category Name" type="text" value="{{ $category_name }}" onkeyup="generateSlug('category_slug',this)">
              </div>
              <div class="form-group">
                <label>Category Slug {{Helper::errorField('category_slug',$errors)}}</label> 
                <input class="form-control" name="category_slug" placeholder="Enter category Slug" type="text" value="{{ $category_slug }}" id="category_slug">
              </div>
              <div class="form-group">
