      ...
      <div class="col-md-12 col-xs-12 col-sm-12">
        <div class="people-img-text">
          ...
          <div class="signinfrom-area">
            <form method="POST" action="{{ route('supplier.color.submit') }}" enctype="multipart/form-data">
              @csrf
              {{-- <input type="hidden" name="categoryId" value="{{ !empty($category) ? $category->id : '' }}"> --}}
              <div class="form-group">
                <label>Color Name {{Helper::errorField('name',$errors)}}</label>
                <input class="form-control" name="name" placeholder="Enter Color Name" type="text" value="{{ $name }}" >
              </div>
              <div class="form-group">
                <input class="btn btn-success" type="submit" value="Save Color">
              </div>
            </form>
          </div>
        </div>
