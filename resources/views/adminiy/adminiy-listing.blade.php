...
@section('content')
<?php 
        if(Request::segment(3) == 'users-listing')
        {
              $listingData->page_heading = 'Seller';
        }
        if(Request::segment(3) == 'products-listing')
        {
              $listingData->page_heading = 'Software';
        }
?>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
      /* jQuery('select[name="connection_type"]').on('change',function(){
        alert();
       })*/
       jQuery('body').on('change','select[name="connection_type"]',function(){
           /* var get =  jQuery('input[name="regapi"').html();
             jQuery('input[name="subsapi"').hide();
             jQuery('input[name="cancelapi"').hide();*/
        });  
    });
</script>