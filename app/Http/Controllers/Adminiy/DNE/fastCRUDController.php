...
class fastCRUDController extends IndexController
{
    ...
    public function index(Request $request){
        if($request->ytable_table == "products"){
            $program_support = implode(",", $request->program_support);
            $request->merge([
                'program_support' => $program_support,
            ]);
            $language_support = implode(",", $request->language_support);
            $request->merge([
                'language_support' => $language_support,
            ]);
            $market_support = implode(",", $request->market_support);
            $request->merge([
